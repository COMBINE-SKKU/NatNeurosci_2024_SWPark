...
%% -- Preparation ------------------------------------------
    ...
    %% 02) Load surfaces & parcellations & colormaps
...
%% Ext Figure 9: Upload data

load('sourceData_ExtFig9_NKI_data_HCPD_rules')
...
% Column 6 is the p-value for each individual comparison. 
     ...
     for for_violin_plot = 1
         ...
         %set(gcf, 'color', 'w'); ylim([0.15 0.75]); grid
         set(gcf, 'color', 'w'); ylim([0 0.7]); grid
         set(gca, 'FontSize', 16); grid on;
         
     end
 
    %% ExtFig 9C: make a surface gradient 
    ...
    BoSurfStatColLim([-0.15 0.15]); colormap((hcp_colormap(:,1:3)));
    project_detection_community_boview(gm_typ.gradients{1}(:,2)*-1,'', schaefer_200_label(1:10242,1)',surfL,1);
    BoSurfStatColLim([-0.15 0.15]); colormap((hcp_colormap(:,1:3)));

    %% ExtFig 9D: segregation index bar graph - individual variance 
%Vis 1:14
%SomMot 15:30
%DorsAttn 31:43
%SalVentAttn 44:54
