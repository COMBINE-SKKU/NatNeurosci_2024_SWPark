...
for for_pathtoolbox = 1
    ...
    PATH2               = '/local_raid3/03_user/shinwon/generative_network_model_analysis/';

    WB_COMMAND = [ 'wb_command' ];
    
    addpath(genpath('/local_raid1/01_software/toolboxes/surfstat/'));
    addpath(genpath('/local_raid1/01_software/freesurfer/matlab/'));
    addpath(genpath('/local_raid1/01_software/toolboxes/npy-matlab/'));
    addpath(genpath('/local_raid1/01_software/HCPpipelines/global/matlab/gifti-1.6'));
    addpath(genpath('/local_raid1/01_software/toolboxes/matlab_util/'));
    addpath(genpath('/local_raid1/01_software/toolboxes/BCT_20190303/'));
    addpath(genpath('/local_raid1/01_software/toolboxes/cifti-matlab/')); 
    %addpath(genpath('/local_raid2/03_user/shinwon/03_software/ENIGMA/matlab/'))    
    addpath(genpath('/local_raid2/03_user/shinwon/03_software/BrainSpace/matlab'));
    %addpath(genpath('/local_raid3/03_user/shinwon/03_software/ComBatHarmonization-master/Matlab/scripts'))
    %addpath(genpath('/local_raid1/01_software/toolboxes/CCS/H3/vistool/surfstat/'));
    addpath('/local_raid2/03_user/shinwon/03_software/npy-matlab');    
    addpath '/local_raid2/03_user/shinwon/01_project/01_thalamusgradient/05_age_effect'
    addpath '/local_raid2/03_user/shinwon/01_project/01_thalamusgradient/05_age_effect_rev'
    addpath '/local_raid2/03_user/shinwon/01_project/01_thalamusgradient/05_analysis_cmap_pmap'
    addpath '/local_raid2/03_user/shinwon/01_project/01_thalamusgradient/05_analysis_cmap_pmap/customcolormap'
    addpath '/local_raid2/03_user/shinwon/01_project/01_thalamusgradient/05_analysis_cmap_pmap/computeCohen_d'
    
end
for for_colormap_load=1
    videenmap = videen(20); videenmap(1:19,:)
    videenmap = [ videenmap; 0.7 0.7 0.7 ];
    %hcp_colormap = readNPY([ '/local_raid2/03_user/shinwon/01_project/01_thalamusgradient/05_age_effect/hcp_colormap.npy' ]);
    ...
    mycol.red2       =   [ones(1,3); ...
        ones(500,1) linspace(0,253,500)'/254 zeros(500,1);...
        ones(64,1) ones(64,1) linspace(0,200,64)'/254];
    hcp_colormap = readNPY('hcp_colormap.npy');
...
for for_load_surfaces = 1
    ...
    temp = gifti([ PATH '01_analysis/03_surface/S900.R.midthickness_MSMAll.10k_fs_LR.surf.gii']);
    ...
    temp = gifti([ PATH '01_analysis/03_surface/S900.L.midthickness_MSMAll.10k_fs_LR.surf.gii']);
    %temp = gifti([ PATH '01_analysis/03_surface/MNI152_T1_1mm.L.very_inflated.10k_fs_LR.surf.gii']);
    ...
    temp = gifti('/local_raid1/01_software/HCPpipelines/global/templates/standard_mesh_atlases/R.atlasroi.10k_fs_LR.shape.gii');
    ...
    for parcel_10k = 1
        for load_schaefers_400_parcel = 1
            schaefer_400 = ciftiopen([ PATH '01_analysis/02_template/Parcellations/Schaefer2018/HCP/fslr10k/cifti/Schaefer2018_400Parcels_7Networks_order_10k.dlabel.nii'],  WB_COMMAND); 
        ...
        for load_schaefers_200_parcel = 1
            schaefer_200 = ciftiopen([ '/local_raid3/03_user/shinwon/generative_network_model_analysis/01_analysis/parcellation/Schaefer2018_200Parcels_7Networks_order.10k.dlabel.nii'],  WB_COMMAND); 
            %figure; plot_hemispheres(schaefer_400.cdata, {surf_lh,surf_rh});
            %figure; SurfStatView(schaefer_400.cdata,surf);
        ...
        for load_yeo_7networks_cortex = 1
           cii_yeo = ciftiopen( '/local_raid2/03_user/shinwon/01_project/01_thalamusgradient/01_analysis/02_template/Parcellations/YeoKrienen2011/fslr10k/RSN-networks.10k_fs_LR.dlabel.nii',  WB_COMMAND);
...
for for_demo_vars = 1
    data = importfile_demodHCPHCPDallAgesqcgrad([PATH '05_age_effect/demo_dHCP,HCPD_allAges_qcgrad.xlsx']);
...
%% Extended Figure 10B: randomly perturb 11 nodes across all regions

    %% upload data 
load('sourceData_ExtFig10B_HCPD_perturbAllRegions.mat')
   ...
   %a3= [r_saldan_0, r_saldan_1, r_saldan_2, r_saldan_3, r_saldan_4]
   ...
   ylim([-0.3 0.5]);
    ...
    percent_int_1 = [(r_saldmn_0 - r_saldmn_1 )/r_saldmn_0] * 100
    ...
    for for_grad0=1
        ...
        BoSurfStatColLim([-0.02 0.03]); colormap((hcp_colormap(:,1:3)));
    end
    for for_grad1=1
